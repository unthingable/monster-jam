...
class TestDevices:
    ...
    def test_device_remote_page_next(self, harness):
        ...
        initial = harness.last_state.get("/state/remote_control/page", {})
        initial_index = initial.get("index", 0)

        harness.send_command("/remote_control/page/next")

        msg = harness.wait_for(
            "/state/remote_control/page",
            predicate=lambda m: m.get("index", -1) != initial_index,
            timeout=2.0,
        )
        assert msg["index"] == initial_index + 1
