...
/** An explicitly invokable action/HardwareBindingSource. Use to construct hardware controls or as generic subscribable
  * action.
  *
  * Being a HardwareBindingSource allows this to be used anywhere a real action is required.
  */
class FakeAction extends HardwareBindingSource[HardwareActionBinding] with Util:
  protected val invokeCallback: () => Unit = () => ()

  val callbacks = mutable.LinkedHashSet.empty[HardwareActionBindable]
  def invoke(): Unit =
    invokeCallback()
    Array.from(callbacks).zipWithIndex.foreach {
      case (f, idx) =>
...
object FakeAction:
  ...
  inline def apply(inline hashString: String): FakeAction = new FakeAction:
    override def hashCode(): Int = hashString.hashCode

class FakeButton(val id: String) extends HasId:
  var isPressed: Boolean = false
  val pressedAction: FakeAction = new FakeAction:
    override val invokeCallback = () => isPressed = true
  val releasedAction: FakeAction = new FakeAction:
    override val invokeCallback = () => isPressed = false
