...
trait Control { this: Jam with MacroL =>
    ...
    new ModeButtonCycleLayer("CONTROL", j.control, CycleMode.Select) with UserControlPages {
      ...
      override val subModes: Vector[ModeLayer] = (
        new SliderBankMode[RemoteControl]("strips remote", page.c.getParameter, identity, Seq.fill(8)(BarMode.DUAL)) {
              ...
              strip.slider.isBeingTouched.addValueObserver(v =>
                if (isOn)
                  touchPage.foreach(tp =>
                    if (idx < tp.getParameterCount)
                      tp.getParameter(idx).value().set(if (v) 1 else 0)
                  )
              )
          ...
          EIGHT.map(idx =>
            new SliderBankMode[CursorDevice](
              s"strips slice $idx",
              ...
              override val modeBindings: Seq[Binding[_, _, _]] = superBindings ++ Vector(
                ...
                EB(
                  j.left.st.release,
                  "slice left release",
                  () => if (pressL && !j.right.st.isPressed) select(0)
                ),
                EB(
                  j.right.st.release,
                  "slice right release",
                  () => if (pressR && !j.left.st.isPressed) select(0)
                ),
              ) ++ EIGHT.flatMap { idx =>
  ...
  lazy val deviceSelector: ModeButtonCycleLayer = new ModeButtonCycleLayer(
    ...
    override def operatedBindings: Iterable[Binding[_, _, _]] =
    ...
    override val subModes: Vector[ModeLayer] = Vector(
      ...
      new SimpleModeLayer("device matrixSelector") {
        ...
        override val modeBindings: Seq[Binding[_, _, _]] = deviceBanks.zipWithIndex.flatMap {
          case (bank, col) =>
            EIGHT.flatMap { row =>
    ...
    override val modeBindings: Seq[Binding[_, _, _]] = super.modeBindings ++ Vector(
      EB(
        ...
        () => if (j.control.st.isPressed) cycle(),
